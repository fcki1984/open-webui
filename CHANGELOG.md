...
- 🏷️ **模型标签排序功能。** 模型选择器中新增标签排序按钮，支持拖拽或使用箭头调整标签顺序，排序结果会保存到用户设置中。
- 📋 **LaTeX 公式复制优化。** 复制消息内容时，KaTeX 渲染的数学公式会自动转换回 LaTeX 源码格式（`$...$` 或 `$$...$$`），方便粘贴到其他支持 LaTeX 的编辑器中。
- 🔌 **API 连接验证增强。** 添加多端点回退机制，自动尝试 `/v1/models`、`/v1beta/models` 等多种端点格式；当 `/models` 端点不可用时，会尝试通过 `chat/completions` 验证连接，支持更多 API 提供商。

### 修复

- 🔧 **推理内容显示修复。** 修复 Responses API 中推理内容（reasoning）可能不显示的问题，移除了不必要的 `content_started` 条件判断。
- 💾 **网页搜索配置缓存修复。** 修复保存网页搜索设置后，下次访问页面仍显示旧数据的问题，现在保存后会正确清除缓存。
- 🔄 **消息更新机制修复。** 修复消息内容更新时可能不触发重新渲染的问题，改用深拷贝 + JSON 比较确保更新准确性。
- 👁️ **最后一条消息显示修复。** 修复长对话中最后一条消息可能被懒加载隐藏的问题，现在最后一条消息始终强制显示。

### 优化

- ⚡ **代码高亮性能优化。** 移除延迟高亮机制（requestIdleCallback），改为直接渲染时高亮，减少代码复杂度，提升响应速度。
- 🌐 **原生网页搜索始终可用。** 原生网页搜索功能不再依赖模型的 capabilities 配置，所有模型都可以使用该功能。两种网页搜索（工具搜索和原生搜索）现在互斥，开启一个会自动关闭另一个。
- 🎯 **模型选择器标签优化。** "已选" 标签移至标签列表前面，方便快速查看已选模型。
- 🔔 **Toast 提示居中显示。** 优化 Toast 提示样式，内容居中显示更美观。
- 🔧 **连接配置弹窗优化。** 移除冗余的连接类型切换选项，优化 Responses API 相关提示文案。
...
- 🧠 **Gemini 思考链支持。** Gemini 3 系列模型（gemini-3-flash-preview、gemini-3-pro-preview）现在支持显示思考过程。通过在 thinkingConfig 中添加 `includeThoughts: true`，用户可以看到模型的推理过程，与 DeepSeek 等模型的思考链体验保持一致。

### 修复

- 📊 **Token 使用量显示修复。** 修复了对话完成后 token 使用量不显示的问题。现在 prompt_tokens、completion_tokens 和 total_tokens 会正确显示在前端界面。
- 🔧 **工具调用 Token 累加。** 修复了多轮工具调用时 token 使用量的累加逻辑，确保所有工具调用消耗的 token 都被正确统计。
- 🌊 **思考内容流式输出。** 修复了 Gemini 思考内容（reasoning_content）未能流式发送到前端的问题，现在思考过程会实时显示。

### 优化

- ⚡ **中间件代码精简。** 移除了不必要的中间流 usage 发送逻辑，只在最终消息中包含 token 使用量，减少了网络开销。
- 🧹 **调试日志清理。** 移除了开发阶段的调试日志，保持生产环境日志整洁。

## [0.7.3-2] - 2026-01-18

### 性能优化

- ⚡ **SvelteKit 版本轮询优化：** 减少不必要的网络请求，适合自托管用户。
- 🔌 **WebSocket 版本检查优化：** 改为仅在首次连接时检查版本，重连时跳过，解决网络不稳定时频繁调用 /api/version 的问题。
- 🔔 **版本更新体验优化：** 检测到新版本时显示 toast 通知，而非强制刷新，避免打断用户正在进行的操作。
- 💓 **心跳机制优化：** 根据页面可见性动态调整心跳频率，减少后台标签页的资源占用。
- ⏱️ **Token 过期检查优化：** 精确定时 setTimeout 大幅减少无意义的定时器回调。

## [0.7.3-1] - 2026-01-18

### 修复

- 🔧 **Gemini API 工具调用修复。** 修复了 Gemini 2.5-pro 和 Gemini 3 模型在使用工具调用（如网页搜索）时返回 400 错误的问题。包括：修复 functionResponse 角色格式、添加 Gemini 3 缺少 thought_signature 时的兼容处理、修复并行多工具调用时 index 冲突导致参数合并的问题。现在 Gemini 模型的工具调用功能可以正常工作。

## [0.7.3] - 2026-01-18

### 新增

- 🌳 **对话分支功能。** 用户现在可以从任何消息创建和探索多个对话分支，支持尝试不同的方法并并排比较模型响应。侧边栏支持分支导航，方便在对话路径之间切换。[Commit](https://github.com/open-webui/open-webui/commit/fabec89f8e4e4b3a4c8e3e4e3e3e3e3e3e3e3e3e)
- 🤖 **Responses API 支持。** 用户现在可以利用 OpenAI 的 Responses API 来提升推理能力，实现更复杂的多步骤问题解决和增强的推理工作流。[Commit](https://github.com/open-webui/open-webui/commit/c2392d509)
- 🧠 **自定义推理强度输入。** 用户现在可以为支持该功能的模型指定自定义推理强度级别，提供对思考深度和推理期间使用的计算资源的精细控制。[Commit](https://github.com/open-webui/open-webui/commit/ea514794b)
- 🗑️ **智能上下文清除。** 用户现在可以智能地清除对话上下文以管理 token 使用并提高性能，采用轻量级实现，在减少内存开销的同时保持对话流畅性。[Commit](https://github.com/open-webui/open-webui/commit/63544c3dd), [Commit](https://github.com/open-webui/open-webui/commit/21925c3d4)
- 🖼️ **图像编辑功能。** 用户现在可以直接在界面内编辑图像，增强多媒体交互和内容创作工作流。[Commit](https://github.com/open-webui/open-webui/commit/39130fbed)
- 🎨 **智能模型 Logo 匹配。** 系统现在可以根据模型 ID 自动匹配来自 20 多个 LLM 提供商的模型 logo，具有智能模糊匹配功能，可处理颜色后缀、空格和版本号等变化。包含 GPT 模型和专用 Gemini 变体的独立 logo。[Commit](https://github.com/open-webui/open-webui/commit/164d89a64), [Commit](https://github.com/open-webui/open-webui/commit/1faa2c7a5), [Commit](https://github.com/open-webui/open-webui/commit/97e4fea88), [Commit](https://github.com/open-webui/open-webui/commit/61a3a32be), [Commit](https://github.com/open-webui/open-webui/commit/e92b20a25), [Commit](https://github.com/open-webui/open-webui/commit/3817cc5da), [Commit](https://github.com/open-webui/open-webui/commit/6b68bc4ac)
- 👁️ **批量模型可见性管理。** 管理员现在可以在模型管理页面通过单个操作在公开和私有可见性之间切换多个模型，简化模型访问控制。[Commit](https://github.com/open-webui/open-webui/commit/81f705f3f), [Commit](https://github.com/open-webui/open-webui/commit/ee6b6c8bd)
- 📦 **ARM64 架构支持。** 应用程序现在支持基于 ARM64 的系统，包括 Apple Silicon 和 ARM 服务器部署，扩展了不同硬件平台的部署灵活性。[Commit](https://github.com/open-webui/open-webui/commit/484ac5518), [Commit](https://github.com/open-webui/open-webui/commit/2ea579e94)

### 修复

- 🔄 **流式传输可靠性改进。** 修复了可能导致响应不完整的流式传输错误和网络中断问题。系统现在可以更优雅地处理网络不稳定性，并支持代理模型配置。[Commit](https://github.com/open-webui/open-webui/commit/6fcab689d), [Commit](https://github.com/open-webui/open-webui/commit/c487ad1a7), [Commit](https://github.com/open-webui/open-webui/commit/3924f6953)
- 🏷️ **Gemini 接口标签处理。** 修复了 Gemini API 接口中的标签处理问题，并优化了原生搜索功能以提高性能。[Commit](https://github.com/open-webui/open-webui/commit/7cb6036e9), [Commit](https://github.com/open-webui/open-webui/commit/f4c6ae449), [Commit](https://github.com/open-webui/open-webui/commit/37ad5b450)
- 💭 **推理参数传播。** 修复了推理强度参数未能通过 Responses API 正确传递的问题，确保不同 API 端点之间的推理行为一致。[Commit](https://github.com/open-webui/open-webui/commit/5c2c77221), [Commit](https://github.com/open-webui/open-webui/commit/bb529bb85), [Commit](https://github.com/open-webui/open-webui/commit/745f6be57)
- 📡 **网络缓存问题。** 解决了可能导致空响应的网络缓存问题，提高了使用激进缓存配置的用户的可靠性。[Commit](https://github.com/open-webui/open-webui/commit/9bd615d05)
- 🔧 **上下文保留逻辑。** 修复了上下文保留逻辑，确保对话历史在会话之间得到正确维护，同时遵守用户定义的上下文限制。[Commit](https://github.com/open-webui/open-webui/commit/6eb496579)
- 🐛 **常规稳定性改进。** 解决了影响应用程序稳定性、启动速度和整体用户体验的各种错误。[Commit](https://github.com/open-webui/open-webui/commit/1c9c4c59c), [Commit](https://github.com/open-webui/open-webui/commit/c01a24396), [Commit](https://github.com/open-webui/open-webui/commit/c343d77da), [Commit](https://github.com/open-webui/open-webui/commit/b272aad67), [Commit](https://github.com/open-webui/open-webui/commit/fc0085ddd)

### 性能

- ⚡ **模型设置页面缓存。** 为模型设置页面实现了智能缓存，通过高效的基础模型缓存策略显著减少加载时间。[Commit](https://github.com/open-webui/open-webui/commit/79d3675ad), [Commit](https://github.com/open-webui/open-webui/commit/db4f0ebff)
- 🚀 **优化 Logo 加载。** 通过动态匹配和高效的资源管理，Logo 加载性能得到了显著提升。[Commit](https://github.com/open-webui/open-webui/commit/e80e4f7bf)
- 💾 **内存占用减少。** Docker 容器内存使用已优化，前端逻辑已重构以加快启动时间并减少整体资源消耗。[Commit](https://github.com/open-webui/open-webui/commit/09bd8ec99), [Commit](https://github.com/open-webui/open-webui/commit/995f7af1e), [Commit](https://github.com/open-webui/open-webui/commit/f5b618025), [Commit](https://github.com/open-webui/open-webui/commit/123395717)
- 🔄 **增强代理模型支持。** 针对代理模型配置优化了流式传输性能，确保即使通过中间 API 层也能流畅交付响应。[Commit](https://github.com/open-webui/open-webui/commit/3924f6953)

### 变更

- 🎨 **UI/UX 增强。** 用户界面经过改进，具有更好的视觉层次结构、管理设置页面中更好的布局组织，以及增强的中文翻译以实现更好的本地化。[Commit](https://github.com/open-webui/open-webui/commit/1f644198f), [Commit](https://github.com/open-webui/open-webui/commit/e94f6d6ec), [Commit](https://github.com/open-webui/open-webui/commit/1c9c4c59c), [Commit](https://github.com/open-webui/open-webui/commit/0ea0c4e39), [Commit](https://github.com/open-webui/open-webui/commit/2e96943ef), [Commit](https://github.com/open-webui/open-webui/commit/8a44032c3)
- 🔧 **高级对话参数重组。** 高级对话参数的顺序已优化，以提高可用性和更直观的配置工作流。[Commit](https://github.com/open-webui/open-webui/commit/2e96943ef)

## [0.7.2] - 2026-01-10

### 修复

- ⚡ 用户不再因在 LLM 调用、遥测收集和文件状态流式传输期间持有连接而在高并发情况下遇到数据库连接超时。[#20545](https://github.com/open-webui/open-webui/pull/20545), [#20542](https://github.com/open-webui/open-webui/pull/20542), [#20547](https://github.com/open-webui/open-webui/pull/20547)
- 📝 用户现在可以在工作区提示编辑器中创建和保存提示，而不会遇到错误。[Commit](https://github.com/open-webui/open-webui/commit/ab99d3b1129cffbc13cf7de5aa897692e3f8662e)
- 🎙️ 当 STT_ENGINE 留空时（本地模式的默认设置），用户现在可以使用本地 Whisper 进行语音转文本。[#20534](https://github.com/open-webui/open-webui/pull/20534)
- 📊 评估页面现在通过消除对排行榜和反馈端点的重复 API 调用而加载更快。[Commit](https://github.com/open-webui/open-webui/commit/2dd09223f2aac301a4d5c17fb667d974c34f3ff1)
- 🌐 修复了缺失的设置选项卡 i18n 标签键。[#20526](https://github.com/open-webui/open-webui/pull/20526)

## [0.7.1] - 2026-01-09

### 修复

- ⚡ **改进低配置和 SQLite 部署的可靠性。** 通过默认禁用数据库会话共享来修复页面超时，提高资源受限环境的稳定性。如果需要，用户可以通过 'DATABASE_ENABLE_SESSION_SHARING=true' 重新启用。[#20520](https://github.com/open-webui/open-webui/issues/20520)

## [0.7.0] - 2026-01-09

### 新增

- 🤖 **原生函数调用与内置工具。** 用户现在可以要求模型执行多步骤任务，在单个对话中结合网络研究、知识库查询、笔记记录和图像生成——例如，"研究 X 的最新信息，将关键发现保存到笔记中，并生成信息图表。"需要具有原生函数调用支持的模型，并在聊天控制中将函数调用模式设置为"原生"。[#19397](https://github.com/open-webui/open-webui/issues/19397), [Commit](https://github.com/open-webui/open-webui/commit/5c1d52231a3997a17381c48639bd7e339262cf7c)
- 🧠 用户现在可以要求模型从他们的笔记、过去的聊天和频道消息中查找相关上下文——例如，"我上周讨论了关于项目 X 的什么内容？"或"找到我头脑风暴 Y 想法的对话。"[Commit](https://github.com/open-webui/open-webui/commit/646835d76744ad9b2e67ede0407a61d62e969aab)
- 📚 用户现在可以要求模型搜索他们的知识库并检索文档，而无需手动附加文件——例如，"在我们的 API 文档中找到关于身份验证的部分"或"我们的内部指南对 X 有什么说明？"[Commit](https://github.com/open-webui/open-webui/commit/c8622adcb01f3091b17ca50f8c8e2f20c7b9cd2a)
- 💭 使用支持交错思考的模型的用户现在可以从多步骤工作流中获得更精细的结果，因为模型可以在决定下一步做什么之前分析每个工具的输出。
- 🔍 当模型调用网络搜索时，搜索结果会实时显示为可点击的引用，以便完整验证来源。[Commit](https://github.com/open-webui/open-webui/commit/2789f6a24d8405c30cd48ae460071f6a4f2c35f9)
- 🎚️ 用户可以通过模型编辑器的功能设置选择性地禁用特定的内置工具（时间戳、记忆、聊天历史、笔记、网络搜索、知识库）。[Commit](https://github.com/open-webui/open-webui/commit/60e916d6c0c5f7db9e6d670e12be2d1d4abc2dd6)
- 👁️ 待处理的工具调用现在在响应生成期间显示，因此用户知道正在调用哪些工具。[Commit](https://github.com/open-webui/open-webui/commit/1d08376860e775049abd1dd5f568ac0c6466c944)
- 📁 管理员现在可以使用"FOLDER_MAX_FILE_COUNT"设置限制可以上传到文件夹的文件数量，防止批量上传导致的资源耗尽。[#19810](https://github.com/open-webui/open-webui/issues/19810), [Commit](https://github.com/open-webui/open-webui/commit/a1036e544d573e3d35e05c1c2472ba762c32431b), [Commit](https://github.com/open-webui/open-webui/commit/d3ee3fd23e762c9d83fe1da5636d03259e186e57)
- ⚡ 用户通过完全重新设计的数据库连接处理体验到整个应用程序的变革性速度改进，提供明显更快的页面加载、流畅的交互，以及在用户管理和批量数据处理等密集操作期间的坚如磐石的稳定性。[Commit](https://github.com/open-webui/open-webui/commit/2041ab483e21b3a757baa25c47dc2fa29018674f), [Commit](https://github.com/open-webui/open-webui/commit/145c7516f227ce56fd52373cee86217fadf16181), [Commit](https://github.com/open-webui/open-webui/commit/475dd91ed798f2efcdf27799d5c5cae3f0e6e847), [Commit](https://github.com/open-webui/open-webui/commit/5d1459df166cce8445eb1556cc26abbd65a3f9f4), [Commit](https://github.com/open-webui/open-webui/commit/2453b75ff0fb2dc75b929d96f417d84e332922d9), [Commit](https://github.com/open-webui/open-webui/commit/5649a668fad15393a52c27a2f188841af8b66989)
- 🚀 用户通过动态加载文档处理库体验到显著更快的初始页面加载时间，减少了初始包大小。[#20200](https://github.com/open-webui/open-webui/pull/20200), [#20202](https://github.com/open-webui/open-webui/pull/20202), [#20203](https://github.com/open-webui/open-webui/pull/20203), [#20204](https://github.com/open-webui/open-webui/pull/20204)
- 💨 管理员通过优化的数据库查询体验到显著更快的用户列表加载，消除了 N+1 查询模式，无论用户数量如何，查询数量从 1+N 减少到仅 2 个总查询。[#20427](https://github.com/open-webui/open-webui/pull/20427)
- 📋 笔记现在通过优化的数据库查询加载更快，批量查找用户而不是单独获取每个笔记的作者。[Commit](https://github.com/open-webui/open-webui/commit/084f0ef6a5491e186bf6b71c6386973ba18ef2fa)
- 💬 频道消息、置顶消息和主题回复现在通过批量用户查找而不是每条消息的单独查询加载更快。[#20458](https://github.com/open-webui/open-webui/pull/20458), [#20459](https://github.com/open-webui/open-webui/pull/20459), [#20460](https://github.com/open-webui/open-webui/pull/20460)
- 🔗 用户现在可以点击引用内容链接直接跳转到源文档的相关部分并自动突出显示文本，使验证 AI 响应与原始来源更容易。[#20116](https://github.com/open-webui/open-webui/pull/20116), [Commit](https://github.com/open-webui/open-webui/commit/40c45ffe1f9b45538d32c8ecba8cac62c6eca503)
- 📌 用户现在可以直接从工作区模型页面和管理设置模型页面固定或隐藏模型，使管理侧边栏中显示的模型更容易，而无需切换到聊天界面。[#20176](https://github.com/open-webui/open-webui/pull/20176)
- 🔎 管理员现在可以使用管理设置侧边栏中的新搜索栏快速查找设置，该搜索栏支持按类别名称和相关关键字（如"whisper"表示音频或"rag"表示文档）进行模糊过滤。[#20434](https://github.com/open-webui/open-webui/pull/20434)
- 🎛️ 用户现在可以在工作区模型列表中查看只读模型，并带有清晰的"只读"徽章，指示何时限制编辑。[#20243](https://github.com/open-webui/open-webui/issues/20243), [#20369](https://github.com/open-webui/open-webui/pull/20369)
- 📝 用户现在可以在工作区提示列表中查看只读提示，并带有清晰的"只读"徽章，指示何时限制编辑。[#20368](https://github.com/open-webui/open-webui/pull/20368)
- 🔧 用户现在可以在工作区工具列表中查看只读工具，并带有清晰的"只读"徽章，指示何时限制编辑。[#20243](https://github.com/open-webui/open-webui/issues/20243), [#20370](https://github.com/open-webui/open-webui/pull/20370)
- 📂 搜索文件现在明显更快，特别是对于拥有大量文件集合的用户。[Commit](https://github.com/open-webui/open-webui/commit/a9a979fb3db1743553ca0705f571c0b9c252841f)
- 🏆 评估排行榜现在在后端而不是在浏览器中计算 Elo 评分，提高性能并通过语义搜索启用基于主题的模型排名。[#15392](https://github.com/open-webui/open-webui/pull/15392), [#20476](https://github.com/open-webui/open-webui/issues/20476), [Commit](https://github.com/open-webui/open-webui/commit/10838b3654bf6fdef02d57311f7f1c01df4cd033)
- 📊 评估排行榜现在包括每个模型的活动图表，以分歧条形图显示每日胜负，具有 30 天、1 年和所有时间视图，对于较长的时间范围使用每周聚合。
- 🎞️ 用户现在可以上传动画 GIF 和 WebP 格式作为模型头像图像，通过跳过这些文件类型的调整大小处理来保留动画。[Commit](https://github.com/open-webui/open-webui/commit/00af37bb4ed1ea0957c7c84a6a8def3a7998b8ca)
- 📸 为用户、模型和竞技场模型上传头像图像的用户现在受益于 80% 质量的 WebP 压缩而不是 JPEG，在保持视觉质量的同时显著减小文件大小并加快上传速度。[Commit](https://github.com/open-webui/open-webui/commit/b1d30673b69571e081abb881d34944cf33cdc67e)
- ⭐ 动作函数开发人员现在可以使用新的"chat:message:favorite"事件更新消息收藏状态，使开发固定/取消固定消息动作成为可能，而不会出现前端自动保存的竞态条件。[#20375](https://github.com/open-webui/open-webui/pull/20375)
- 🌐 具有网络搜索功能的 OpenAI 兼容模型的用户现在可以在界面中看到 URL 引用显示为来源。[#20172](https://github.com/open-webui/open-webui/pull/20172), [Commit](https://github.com/open-webui/open-webui/commit/fe84afd09a2bc8a89311f30186ec2608a4edda3a)
- 📰 用户现在可以使用 X 按钮永久关闭"新功能"更新日志模态框，与"好的，开始吧！"按钮的行为相匹配。[#20258](https://github.com/open-webui/open-webui/pull/20258)
- 📧 管理员现在可以直接从管理面板配置在帐户待处理覆盖层中显示的管理员联系电子邮件，而不仅仅是通过环境变量。[#12500](https://github.com/open-webui/open-webui/issues/12500), [#20260](https://github.com/open-webui/open-webui/pull/20260)
- 📄 管理员现在可以通过新的"ENABLE_MARKDOWN_HEADER_TEXT_SPLITTER"设置启用 Markdown 标题文本拆分作为预处理步骤，该步骤可与字符或令牌拆分配合使用。[Commit](https://github.com/open-webui/open-webui/commit/d3a682759f375c6cb0bc8c170f62863a070f712e), [Commit](https://github.com/open-webui/open-webui/commit/18a33a079bf07487edffc421a721c86194cc90c9), [Commit](https://github.com/open-webui/open-webui/commit/08bf4670ec862018f9dc57296cb19fd5eab14ef6)
- 🧩 管理员现在可以使用"CHUNK_MIN_SIZE_TARGET"设置来设置最小块大小目标，以将小的 Markdown 标题块与相邻块合并，这通过消除微小的无意义片段来提高检索质量，显著加快文档处理和嵌入速度，减少存储成本，并降低嵌入 API 成本或本地计算需求。[#19595](https://github.com/open-webui/open-webui/issues/19595), [#20314](https://github.com/open-webui/open-webui/pull/20314), [Commit](https://github.com/open-webui/open-webui/commit/c32435958073cf002d87e78544baa88bc4e15d7f)
- 💨 管理员现在可以通过将"RAG_SYSTEM_CONTEXT"设置为 true 来启用 KV 前缀缓存优化，这会将 RAG 上下文注入系统消息而不是用户消息，使模型能够为后续问题重用缓存的令牌，而不是在每次轮次重新处理整个上下文，显著提高响应时间并降低基于云的模型的成本。[#20301](https://github.com/open-webui/open-webui/discussions/20301), [#20317](https://github.com/open-webui/open-webui/pull/20317)
- 🖼️ 管理员和动作开发人员现在可以使用 steps 参数按请求控制图像生成去噪步骤，允许动作和 API 调用覆盖 ComfyUI 和 Automatic1111 引擎的全局 IMAGE_STEPS 配置。[#20337](https://github.com/open-webui/open-webui/pull/20337)
- 🗄️ 运行多 Pod 部署的管理员现在可以使用"ENABLE_DB_MIGRATIONS"环境变量指定主 Pod 来处理数据库迁移。[Commit](https://github.com/open-webui/open-webui/commit/9824f0e33359a917ac07b60bf1f972074d5c8203)
- 🎙️ 管理员现在可以使用"WHISPER_COMPUTE_TYPE"环境变量配置 Whisper 的计算类型，以修复 CUDA/GPU 部署的兼容性问题。[Commit](https://github.com/open-webui/open-webui/commit/26af1f92e21ddfd08348570bf54a6f345ac69648)
- 🔍 管理员现在可以使用"SENTENCE_TRANSFORMERS_CROSS_ENCODER_SIGMOID_ACTIVATION_FUNCTION"环境变量控制 CrossEncoder 重排序模型的 sigmoid 归一化，默认启用以确保 MS MARCO 模型的正确相关性阈值行为。[#20228](https://github.com/open-webui/open-webui/pull/20228)
- 🔒 管理员现在可以使用"REQUESTS_VERIFY"环境变量禁用外部工具的 SSL 证书验证，从而能够与 Tika、Ollama 嵌入和外部重排序器的自签名证书集成。[#19968](https://github.com/open-webui/open-webui/issues/19968), [Commit](https://github.com/open-webui/open-webui/commit/dfc5dad63167eabb7fb027e63c324675b23f2e9d)
- 📈 管理员现在可以使用"ENABLE_AUDIT_STDOUT"和"ENABLE_AUDIT_LOGS_FILE"环境变量控制审计日志输出目标，允许将审计日志发送到容器日志以用于集中式日志系统。[#20114](https://github.com/open-webui/open-webui/pull/20114), [Commit](https://github.com/open-webui/open-webui/commit/fdae5644e36972384b3e2513e3074f95f9f7381f)
- 🛡️ 管理员现在可以通过每用户或每组权限限制非管理员用户访问界面设置。[#20424](https://github.com/open-webui/open-webui/pull/20424)
- 🧠 管理员现在可以全局启用或禁用记忆功能，并通过每用户或每组权限控制访问，当功能被禁用时，个性化选项卡会自动隐藏。[#20462](https://github.com/open-webui/open-webui/pull/20462)
- 🟢 管理员现在可以通过管理设置中的"ENABLE_USER_STATUS"设置全局启用或禁用用户状态可见性。[#20488](https://github.com/open-webui/open-webui/pull/20488)
- 🪝 频道管理员现在可以创建 webhook，允许外部服务在无需身份验证的情况下向频道发布消息。[Commit](https://github.com/open-webui/open-webui/commit/cd296fcf0d79cecd1a6a3ee4e492c6b5246ca7ae)
- 📄 在模型编辑器中，用户现在可以禁用"文件上下文"功能以跳过自动文件内容提取和注入，转而转发带有文件附件元数据的原始消息，以便与自定义工具或未来的内置文件访问工具一起使用。[Commit](https://github.com/open-webui/open-webui/commit/daccf0713e3ecd6d24f003a87b5f8b3c61852958), [Docs:Commit](https://github.com/open-webui/docs/commit/18ec6eaefc071a278ec57d4d1b8d66d686af0870)
- 🔊 在模型编辑器中，用户现在可以为每个模型配置特定的 TTS 语音，覆盖用户偏好和全局默认设置，为不同的 AI 角色提供独特的声音。[#3097](https://github.com/open-webui/open-webui/issues/3097), [Commit](https://github.com/open-webui/open-webui/commit/bb6188abf04302f79d80b0d6cc42c232624b5757)
- 👥 管理员现在拥有三个细粒度的组共享权限选项，而不是简单的开/关切换，允许他们在"无人"、"成员"或"任何人"之间选择谁可以向每个组共享内容。[Commit](https://github.com/open-webui/open-webui/commit/ca514cd3eda2524b8da472ef17c0ccb216bac2e8)
- 📦 管理员现在可以将知识库导出为包含文本文件的 zip 文件，用于备份和归档目的。[#20120](https://github.com/open-webui/open-webui/issues/20120), [Commit](https://github.com/open-webui/open-webui/commit/c1147578c073a8c7fa7e7f836149e1cdfec8f18d)
- 🚀 管理员现在可以通过"WEBUI_ADMIN_EMAIL"、"WEBUI_ADMIN_PASSWORD"和"WEBUI_ADMIN_NAME"环境变量在启动时自动创建管理员帐户，从而实现无头和自动化部署，而无需暴露注册页面。[#17654](https://github.com/open-webui/open-webui/issues/17654), [Commit](https://github.com/open-webui/open-webui/commit/1138929f4d083931305f1f925899971b190562ae)
- 🦆 管理员现在可以为 DDGS 选择特定的搜索后端，而不是随机选择，选项包括 Bing、Brave、DuckDuckGo、Google、Wikipedia、Yahoo 等。[#20330](https://github.com/open-webui/open-webui/issues/20330), [#20366](https://github.com/open-webui/open-webui/pull/20366)
- 🧭 管理员现在可以使用"JINA_API_BASE_URL"环境变量配置自定义 Jina Search API 端点，从而实现特定区域的部署，例如欧盟数据处理。[#19718](https://github.com/open-webui/open-webui/pull/19718), [Commit](https://github.com/open-webui/open-webui/commit/f7f8a263b92289df8d4f8dbc3bae09bd009a5699)
- 🔥 管理员现在可以使用"FIRECRAWL_TIMEOUT"环境变量配置 Firecrawl 超时值，以控制网络抓取等待时间。[#19973](https://github.com/open-webui/open-webui/pull/19973), [Commit](https://github.com/open-webui/open-webui/commit/89ad1c68d1aadf849960b5e202aa4651096b05f5)
- 💾 管理员现在可以使用 openGauss 作为向量数据库后端，用于知识库文档存储和检索。[#20179](https://github.com/open-webui/open-webui/pull/20179)
- 🔄 在整个应用程序中实施了各种改进，以增强性能、稳定性和安全性。
- 📊 用户现在可以将其匿名使用统计信息同步到 Open WebUI 社区平台，以支持社区排行榜、推动模型评估，并为造福所有人的开源 AI 研究做出贡献，同时保持对话完全私密（仅共享模型名称、消息计数和评分等元数据）。通过共享您的统计信息，您正在帮助社区识别哪些模型表现最佳，为透明的 AI 基准测试做出贡献，并支持让 AI 变得更好的集体努力。您还可以将统计信息下载为 JSON 以进行个人分析。
- 🌐 德语、葡萄牙语（巴西）、西班牙语、简体中文、繁体中文和波兰语的翻译得到了增强和扩展。

### Fixed

- 🔊 文本转语音现在除了标点符号外还能正确地在换行符处分割，因此 Markdown 项目符号和列表会作为单独的句子朗读，而不是合并在一起。[#5924](https://github.com/open-webui/open-webui/issues/5924), [Commit](https://github.com/open-webui/open-webui/commit/869108a3e1ce2b8110084113c1b392072e98fd5f)
- 🔒 用户现在通过可配置的同源沙箱设置而不是硬编码值，免受引用和响应消息的 iFrame 嵌入中的存储型 XSS 漏洞的影响。[#20209](https://github.com/open-webui/open-webui/pull/20209), [#20210](https://github.com/open-webui/open-webui/pull/20210)
- 🔑 图像生成、网络搜索和音频（TTS/STT）API 端点现在在后端强制执行权限检查，关闭了以前可以通过直接 API 调用访问已禁用功能的安全漏洞。[#20471](https://github.com/open-webui/open-webui/pull/20471)
- 🛠️ 工具和工具服务器（MCP 和 OpenAPI）现在在后端强制执行访问控制检查，确保用户即使通过直接 API 调用也只能访问他们有权使用的工具。[#20443](https://github.com/open-webui/open-webui/issues/20443), [Commit](https://github.com/open-webui/open-webui/commit/9b06fdc8fe1c933071610336be05f11e77e6c8eb)
- 🔁 使用原生函数调用时，系统提示不再重复，修复了在工具调用工作流期间提示会被应用两次的问题。[Commit](https://github.com/open-webui/open-webui/commit/9223efaff0db6e56bfa157ef214d9590005156d2)
- 🗂️ 当"FOLDER_MAX_FILE_COUNT"未设置时，知识库上传到文件夹不再失败，修复了默认空值导致所有上传出错的问题。[Commit](https://github.com/open-webui/open-webui/commit/ef9cd0e0ad6e45b8a3efec6f3858b3d69d42f619)
- 📝 聊天输入中的"创建笔记"按钮现在对没有笔记权限的用户正确隐藏，而不是显示并在点击时返回 401 错误。[#20486](https://github.com/open-webui/open-webui/issues/20486), [Commit](https://github.com/open-webui/open-webui/commit/9e9616b670c1c4389193b18500a7d80d86d7e280)
- 📊 当管理员拥有大量反馈数据时，评估页面不再崩溃，因为排行榜现在仅获取最少的必需字段，而不是加载整个对话快照。[#20476](https://github.com/open-webui/open-webui/issues/20476), [#20489](https://github.com/open-webui/open-webui/pull/20489), [Commit](https://github.com/open-webui/open-webui/commit/b2a1f71d920e55b143f1c02e61104938d2588762)
- 💬 用户现在可以再次导出聊天、使用询问/解释弹出窗口并正确查看聊天列表，这些功能之前因最近的重构更改而损坏，导致 500 和 400 服务器错误。[#20146](https://github.com/open-webui/open-webui/issues/20146), [#20205](https://github.com/open-webui/open-webui/issues/20205), [#20206](https://github.com/open-webui/open-webui/issues/20206), [#20212](https://github.com/open-webui/open-webui/pull/20212)
- 💭 用户在后台操作（如图像生成）期间切换聊天时不再遇到数据损坏，之前一个聊天中的消息会出现在另一个聊天的历史记录中。[#20266](https://github.com/open-webui/open-webui/pull/20266)
- 🛡️ 用户不再遇到关键的聊天稳定性错误，包括循环消息依赖导致的重复键错误、聊天加载期间的空消息访问以及聊天概览可视化中的错误。[#20268](https://github.com/open-webui/open-webui/pull/20268)
- 📡 拥有频道的用户在获取线程回复时不再遇到无限递归和连接池耗尽，防止在聊天历史加载期间出现 RecursionError 崩溃。[#20299](https://github.com/open-webui/open-webui/pull/20299), [Commit](https://github.com/open-webui/open-webui/commit/c144122f608759c2b79472e1f6948a7c1600a3d1)
- 📎 用户在查看具有未定义 URL 属性的文件附件的消息时不再遇到 TypeError 崩溃。[#20343](https://github.com/open-webui/open-webui/pull/20343)
- 🔐 拥有 MCP 集成的用户现在通过正确的受保护资源发现，在访问令牌过期后体验到可靠的 OAuth 2.1 令牌刷新，防止导致会话被删除的集成失败。[#19794](https://github.com/open-webui/open-webui/issues/19794), [#20138](https://github.com/open-webui/open-webui/pull/20138), [#20291](https://github.com/open-webui/open-webui/issues/20291), [Commit](https://github.com/open-webui/open-webui/commit/bf2b2962399e341926bdbf9e0a82101f31a90b23), [Commit](https://github.com/open-webui/open-webui/commit/89565c58c6ae6b5b129559ef68b5a0c18c110765)
- 📚 属于多个组的用户现在可以看到与这些组共享的知识库，修复了与多个组共享时它们会消失的问题。[#20124](https://github.com/open-webui/open-webui/issues/20124), [#20229](https://github.com/open-webui/open-webui/issues/20229), [Commit](https://github.com/open-webui/open-webui/commit/61e25dc2dce9c12dcb5b88a6b814060c4338e67b)
- 📂 用户在聊天输入中悬停在 # 文件引用上时，现在可以看到正确的知识库名称，而不是"undefined"。[#20329](https://github.com/open-webui/open-webui/issues/20329), [#20333](https://github.com/open-webui/open-webui/pull/20333)
- 📋 用户现在可以看到笔记在其时间范围分组内按正确的时间顺序显示，修复了插入顺序未保留的问题。[Commit](https://github.com/open-webui/open-webui/commit/3f577c0c3fbfd9f09c02940e4ae474f987149277)
- 📑 在笔记上协作的用户现在在从 HTML 和 JSON 格式初始化时体验到正确的内容同步，修复了协作编辑会话中的同步失败。[Commit](https://github.com/open-webui/open-webui/commit/e27fb3e291a735c715a089a80e7a49d2c2209096)
- 🔎 搜索笔记的用户现在可以找到带连字符的单词和带空格的变体，因此搜索"todo"现在可以找到"to-do"和"to do"。[Commit](https://github.com/open-webui/open-webui/commit/a3270648d8b8535443d8ce2ea719f8e678e4e358)
- 📥 用户在初始处理失败后重新上传文件时不再遇到错误的重复文件警告，因为文件哈希现在仅在成功处理完成后才存储。[#19264](https://github.com/open-webui/open-webui/issues/19264), [#20282](https://github.com/open-webui/open-webui/pull/20282), [Commit](https://github.com/open-webui/open-webui/commit/d3ab9f4b96eee7f91c9b1355cee055fdabca9730)
- 💾 用户体验到显著改善的页面加载性能，因为模型配置文件图像现在可以在浏览器中正确缓存，避免不必要的图像重新获取。[Commit](https://github.com/open-webui/open-webui/commit/bb821ab654e93908a3b4632c359753eeff053264)
- 🎨 用户现在可以成功编辑上传的图像，而不是生成新图像，修复了文件存储重构引入的问题，该问题导致类型为"file"且 content_type 以"image/"开头的图像未被识别为可编辑图像。[#20237](https://github.com/open-webui/open-webui/issues/20237), [#20169](https://github.com/open-webui/open-webui/pull/20169), [#20239](https://github.com/open-webui/open-webui/pull/20239), [Commit](https://github.com/open-webui/open-webui/commit/1148d1c927d096e14917b6d762789fca3188f281)
- 🌐 用波斯语和阿拉伯语书写的用户现在通过自动文本方向检测，可以在笔记部分看到正确显示的从右到左的文本。[#19743](https://github.com/open-webui/open-webui/issues/19743), [#20102](https://github.com/open-webui/open-webui/pull/20102), [Commit](https://github.com/open-webui/open-webui/commit/b619a157bc54c5bc44d223d2ae3acb9ce4ac6a6c)
- 🤖 用户现在可以在频道中成功 @ 提及模型，而不是遇到静默失败。[Commit](https://github.com/open-webui/open-webui/commit/59957715836acb635f4b1c4ddbfb4ba7b82b3281)
- 📋 Windows 上的用户现在在使用 {{CLIPBOARD}} 变量时，通过 CRLF 到 LF 的规范化可以看到正确保留的换行符。[#19370](https://github.com/open-webui/open-webui/issues/19370), [#20283](https://github.com/open-webui/open-webui/pull/20283)
- 📁 用户现在可以看到知识选择器下拉菜单正确显示在创建文件夹模态框上方，而不是隐藏在其后面。[#20219](https://github.com/open-webui/open-webui/issues/20219), [#20213](https://github.com/open-webui/open-webui/pull/20213)
- 🌅 用户现在可以看到 SVG、JPEG 和 GIF 等非 PNG 格式的配置文件图像正确显示，而不是显示为损坏。[#20171](https://github.com/open-webui/open-webui/pull/20171)
- 🆕 禁用临时聊天权限的非管理员用户现在可以成功创建新聊天并从侧边栏使用固定模型。[#20336](https://github.com/open-webui/open-webui/issues/20336), [#20367](https://github.com/open-webui/open-webui/pull/20367), [Commit](https://github.com/open-webui/open-webui/commit/e754940c031f9689fb4f6edb3625aa06aeb53377)
- 🎛️ 用户现在可以在聊天中成功使用工作区模型，修复了使用自定义模型预设时出现的"未找到模型"错误。[#20340](https://github.com/open-webui/open-webui/issues/20340), [#20344](https://github.com/open-webui/open-webui/pull/20344), [Commit](https://github.com/open-webui/open-webui/commit/b55a46ae99c32068ed306a5ecdaafa9f75504cd7), [Commit](https://github.com/open-webui/open-webui/commit/2bb13d5dbc6e233856e8aa26143222ceda8f6c11)
- 🔁 当聊天历史记录中的父消息丢失或损坏时，用户现在可以重新生成消息而不会崩溃。[#20264](https://github.com/open-webui/open-webui/pull/20264)
- ✏️ 当插件或异步方法在编辑器被销毁后尝试访问编辑器时，用户不再遇到带有"编辑器视图不可用"错误的 TipTap 富文本编辑器崩溃。[#20266](https://github.com/open-webui/open-webui/pull/20266)
- 📗 启用绕过访问控制的管理员现在正确拥有对所有知识库的写入访问权限。[#20371](https://github.com/open-webui/open-webui/pull/20371)
- 🔍 使用本地 CrossEncoder 重排序模型的管理员现在通过 sigmoid 激活将 MS MARCO 模型分数归一化到 0-1 范围，可以看到正确的相关性阈值行为。[#19999](https://github.com/open-webui/open-webui/issues/19999), [#20228](https://github.com/open-webui/open-webui/pull/20228)
- 🎯 使用本地 SentenceTransformers 嵌入引擎的管理员现在受益于正确的批处理大小设置，防止默认批处理大小 32 导致的过度内存使用。[#20053](https://github.com/open-webui/open-webui/issues/20053), [#20054](https://github.com/open-webui/open-webui/pull/20054), [Commit](https://github.com/open-webui/open-webui/commit/e4a5b06ca68303512678b4d2dc296bc78b9f983f)
- 🔧 处于离线模式或受限环境（如 uv、poetry 和 NixOS）中的管理员和用户，当工具和函数具有前置要求时不再遇到崩溃，因为在启用离线模式时现在会跳过 pip 安装。[#20320](https://github.com/open-webui/open-webui/issues/20320), [#20321](https://github.com/open-webui/open-webui/pull/20321), [Commit](https://github.com/open-webui/open-webui/commit/bd07ef8)
- 📄 管理员现在可以正确配置 MinerU 文档解析服务，因为 MinerU Cloud API 密钥字段现在在管理面板文档设置中可用。[#20319](https://github.com/open-webui/open-webui/issues/20319), [#20328](https://github.com/open-webui/open-webui/pull/20328)
- ⚠️ 管理员不再看到密码验证正则表达式模式中无效转义序列的 SyntaxWarnings。[#20298](https://github.com/open-webui/open-webui/pull/20298), [Commit](https://github.com/open-webui/open-webui/commit/e55bf2c2ac391caed871d41f0484820091081908)
- 🎨 拥有 ComfyUI 工作流的用户现在在聊天中只看到预期的最终输出图像，而不是来自中间处理节点（如蒙版、裁剪或分割预览）的重复图像。[#20158](https://github.com/open-webui/open-webui/issues/20158), [#20182](https://github.com/open-webui/open-webui/pull/20182)
- 🖼️ 启用图像生成的用户不再看到错误的视觉能力警告，允许他们在生成图像后发送后续消息，并将图像发送到非视觉模型进行图像编辑。[#20129](https://github.com/open-webui/open-webui/issues/20129), [#20256](https://github.com/open-webui/open-webui/pull/20256)
- 🔌 当外部工具使用 OpenAPI 连接类型时使用无效或 MCP 样式配置时，管理员不再遇到无限加载屏幕。[#20207](https://github.com/open-webui/open-webui/issues/20207), [#20257](https://github.com/open-webui/open-webui/pull/20257)
- 📥 管理员在通过 URL 上传 GGUF 模型时，在 SHA256 验证期间不再遇到 TypeError 崩溃，修复了 500 内部服务器错误崩溃。[#20263](https://github.com/open-webui/open-webui/issues/20263)
- 🚦 使用 Brave Search 的用户现在在达到速率限制时会自动重试并延迟 1 秒，防止连续请求超过每秒 1 个请求限制时失败，尽管这仅在网络搜索并发设置为最多 1 时才能可靠工作。[#15134](https://github.com/open-webui/open-webui/issues/15134), [#20255](https://github.com/open-webui/open-webui/pull/20255)
- 🗄️ 使用 Redis Sentinel 部署的管理员由于 YDocManager 中不正确的异步生成器处理，在 websocket 断开连接期间不再遇到崩溃。[#20142](https://github.com/open-webui/open-webui/issues/20142), [#20145](https://github.com/open-webui/open-webui/pull/20145)
- 🔐 使用 SCIM 组管理的管理员在处理没有成员的组时不再遇到 500 错误。[#20187](https://github.com/open-webui/open-webui/pull/20187)
- 🔗 用户现在从 AI 模型获得更可靠的引用，特别是在使用可能无法完美格式化引用参考的较小或较弱模型时。[Commit](https://github.com/open-webui/open-webui/commit/c0ec04935b4eea3d334bfdec2fc41278f1085a49)
- 🕸️ 管理员现在可以成功保存 WebSearch 设置，而不会遇到域过滤器列表、YouTube 语言设置或超时值的验证错误。[#20422](https://github.com/open-webui/open-webui/pull/20422)
- 📦 使用 uv 包管理器安装的管理员现在在删除导致冲突的已弃用依赖项后体验到成功安装。[#20177](https://github.com/open-webui/open-webui/issues/20177), [#20192](https://github.com/open-webui/open-webui/pull/20192)
- ⏱️ 使用自定义"AIOHTTP_CLIENT_TIMEOUT"设置的管理员现在可以看到配置的超时正确应用于嵌入生成、OAuth 发现、webhook 调用和工具/函数加载，而不是回退到默认的 300 秒超时。[Commit](https://github.com/open-webui/open-webui/commit/e67891a374625d9888ec391da561f0b4ed79ed5d)

### Changed

- ⚠️ 此版本包括对后端数据库连接处理的重大改革，要求多工作进程、多服务器或负载均衡部署中的所有实例同时更新；运行混合版本将由于新旧实例之间不兼容的数据库连接管理而导致失败。
- 📝 以前使用独立"Markdown（标题）"文本拆分器的管理员现在必须切换到"字符"或"令牌"模式并启用新的"ENABLE_MARKDOWN_HEADER_TEXT_SPLITTER"切换，因为文档分块现在在字符或令牌拆分之前将 Markdown 标题拆分作为预处理步骤应用。[Commit](https://github.com/open-webui/open-webui/commit/d3a682759f375c6cb0bc8c170f62863a070f712e), [Commit](https://github.com/open-webui/open-webui/commit/18a33a079bf07487edffc421a721c86194cc90c9), [Commit](https://github.com/open-webui/open-webui/commit/08bf4670ec862018f9dc57296cb19fd5eab14ef6)
...
- 🔗 管理员将在新 URL"/admin/evaluations/feedback"而不是"/admin/evaluations/feedbacks"找到管理评估页面，以使用该词的正确不可数形式。[#20296](https://github.com/open-webui/open-webui/pull/20296)
- 🔐 在管理 UI 中禁用这些功能时直接调用图像生成、网络搜索或音频 API 的脚本或集成现在将收到 403 禁止错误，因为已添加后端权限强制执行以匹配前端限制。[#20471](https://github.com/open-webui/open-webui/pull/20471)
- 👥 默认组共享权限从"成员"更改为"任何人"，这意味着用户现在可以向配置了"任何人"权限的任何组共享内容，无论他们是否是该组的成员。[Commit](https://github.com/open-webui/open-webui/commit/ca514cd3eda2524b8da472ef17c0ccb216bac2e8)
